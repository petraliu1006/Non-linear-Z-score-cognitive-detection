...
mxval <- 30 ## maximum value for the outcome
...
chopvals <- function(x){
  x <- x[!(x$NACCAGE == 40 & x$residuals < (-2.5)), ]
  x <- x[!(x$NACCAGE > 40 & x$NACCAGE <= 46 & x$residuals < (-0.5)), ]
  x <- x[!(x$NACCAGE >= 47 & x$NACCAGE <= 52 & x$residuals < (-2.5)), ]
  x
